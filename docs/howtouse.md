...
- `packaging_type`:: currently it accepts two options, `separated_dir` and `bundle`, vdist defaults to `separated_dir`. While `separated_dir` keeps your proyect folder apart from your compiled Python folder (your project in a folder with the "app" name and intepreter in a folder called "vdist-python"), you may consider "bundle" if your application was able to be installed in "site-packages" Python folder. If you set "bundle", only one self-sufficient folder is going to be packaged (with "app" name): the compiled Python folder with your application installed in its "site-packages" folder. If you use "bundle" you should prepare a post-install script to copy entry points from compiled Python "bin" folder to a system folder like "/usr/bin", this script should be included in your package using "fpm_args" parameter.
